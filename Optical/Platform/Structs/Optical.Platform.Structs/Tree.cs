...
namespace Optical.Platform.Structs
{
    ...
    public class TreeElement<T> : ICloneable
    {
        ...
        public TreeElement(object name, TreeElement<T>? parent = null)
        {
            ...
            {
                throw new ArgumentNullException(nameof(name), "要素名がNullまたは空文字です。");
            }
        ...
        /// 子要素を所有しているかを表します。
        /// </summary>
        public bool HasChild => (!Child.IsEmpty);
        ...
        private TreeElement<T>? searchChild(string path)
        {
            ...
            {
                throw new ArgumentNullException(nameof(path), "要素パスがNullまたは空文字です。");
            }
        ...
        public void RemoveChild(string path)
        {
            ...
            removeNode.ClearChild();
            if (removeNode.IsRoot)
            {
                return;
            }

            removeNode.Parent?.Child.TryRemove(removeNode.Name, out _);
        }
